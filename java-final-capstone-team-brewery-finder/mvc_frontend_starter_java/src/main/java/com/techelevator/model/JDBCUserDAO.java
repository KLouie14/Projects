...
public class JDBCUserDAO implements UserDAO {
    ...
    public boolean userHasRole(String role, String userName) {
        User currentUser = getUserByUserName(userName);         // Get the current user from session map or null if not logged in
        if (currentUser != null && role != null) {  // if user is logged in and role String is not empty...
            return role.contains(currentUser.getRole());  // return whether the roles passed in is the role of the current user
        } else {
